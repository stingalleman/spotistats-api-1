...
exports.followPlaylist = void 0;
const node_fetch_1 = __importDefault(require("node-fetch"));
const misc_1 = require("../misc");
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function followPlaylist(accessToken, playlistID) {
    const req = await node_fetch_1.default(`https://api.spotify.com/v1/playlists/${playlistID}/followers`, {
        method: "PUT",
        headers: {
    ...
    if (req.status !== 200) { // if request not 200
        misc_1.errorLogger(`unexpected error! (followPlaylist)\nstatus: ${req.status}\nerror: ${await req.text()}`);
    }
    else {
        return "success";
    }
